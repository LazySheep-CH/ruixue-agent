...
⚠ 仍然存在的偏差(合成评测集的天花板,必须知道):
    - 出题时 LLM 看着原文,措辞会向原文靠 → 分数偏高
    - 只标一个 gold chunk;真有别的段落也能答,会被冤枉判 0
...
def main() -> None:
    ...
    #
    # 踩过的坑:第一版随机抽 chunk → 标准规范只占到 8/150(5%)。
