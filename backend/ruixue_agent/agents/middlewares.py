...
create_agent(middleware=[...]) 接收它们。deer-flow 的中间件链也是这么叠的。
钩子(和你 mini-deerflow 写过的一样):
...
class PromptInjectionGuardMiddleware(AgentMiddleware):
    ...
    为什么不直接拒绝:正则判定必然有误差,把真实用户挡在门外的代价,
    ...
    def before_model(self, state, runtime):
        ...
        if not messages:
            return None
        last = messages[-1]
        # 只检最后一条用户消息:历史消息已检过,工具返回的内容由 RAG 侧边界隔离
...
# 记忆注入的标题。和技能的 SKILL_HEADER 同理:它是约定好的标记,
...
class MemoryRecallMiddleware(AgentMiddleware):
    ...
    有人会把记忆拼进用户消息里假装"用户刚说的"。那样做有两个坏处:
      1) 模型分不清哪句是用户现在说的、哪句是系统补的背景;
      2) 万一记忆里被写入了恶意内容,它就以"用户指令"的身份进了上下文 ——
         而用户消息的指令权重远高于系统背景。
    所以必须作为系统背景注入,并明确标注"这是历史背景,不是本次请求"。

    取不到 user_id 就不注入:
    记忆是按用户隔离的。拿不到身份时宁可不注入,绝不能"给个默认用户" ——
    ...
    def before_model(self, state, runtime):
        ...
        # 扫全部消息:配合 _last_human,本方法在同一轮里可能被调用多次。
...
def _last_human(messages):
    """取最后一条用户消息;没有就返回 None。

    注意:2026-08-12 修:不能用 messages[-1]:
    原来两个注入中间件都写的是:
...
def _user_id_from(runtime) -> str:
    ...
    注意:2026-08-12 修:原来读 `runtime.config`,而它根本不存在:
    ...
    1) 失败方式是"什么都不做":没身份就不注入,这条分支本身是对的
       (宁可不给,不可给错人),所以没有任何报错、没有异常日志,
       只是记忆功能静静地不生效。

    2) 测试是假的:`tests/test_memory.py` 里的 `_Rt` 是手搓的假 runtime,
...
# 工具失败消息的机器可读标记。
#
# 为什么要单独抽一个常量:评测需要区分工具挂了和模型没选对工具——
