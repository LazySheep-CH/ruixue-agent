"""长期记忆收益实验:对比"有记忆/没记忆"两组的任务完成率。

设计要点:

- 每道题两轮,setup 轮陈述事实、probe 轮提一个必须用上该事实的问题,
  两轮用不同 thread_id。同一 thread 里 checkpointer 会把上下文带过去,
  测出来的是短期记忆;换 thread 后上下文归零,probe 轮只能靠长期记忆。
- 两组唯一的差别是 recall 是否返回内容,中间件链等其他条件不动,
  否则差值说不清归因。写记忆两组都照做(off 组反正读不到)。
- 两组用不同 user_id。记忆是软删 + 内容寻址,删过的不复活,共用 user
  靠删除重置会让第二组永远存不进去,实验静默失效。
- probe 里不允许出现要点答案(加载期校验,见 _validate),否则不用记忆
  也能答对,这道题测不出任何东西。

2026-08-12 首次运行就发现记忆从未注入过(runtime.config 不存在),
修复过程见 agents/middlewares.py。
"""
