...
# 用户问题放在资料之后:靠近末尾的指令权重更高,可削弱资料尾部藏指令的效果。
_USER = """{context}
...
class Generator:
    ...
    # 那条理由只对agent 自己的模型调用成立,因为它跑在 ModelRetryMiddleware 下面。
    # 而这里是 search_knowledge 内部的第二次模型调用,不在任何中间件之下:
