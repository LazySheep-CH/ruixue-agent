...
为什么要转:
  1) embedding 看到 <table><tr><td> 是噪声,看不懂表格(和 LaTeX 那个问题一样)
  2) 实测 84% 的表格 HTML 超过 400 字符 → 整块 embedding 会被【悄悄截断】,大半内容检索不到
  3) 单元格里还混着行内 LaTeX($\\mathrm{P}_{2}0_{5}$ 其实是 P2O5)——
