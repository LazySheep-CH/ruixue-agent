...
原则:确定性的部分榨干在工具里,只把权衡取舍留给 LLM。

注意:️ 已知局限:厚度不能作为决策维度(实测结论,必须如实披露)
