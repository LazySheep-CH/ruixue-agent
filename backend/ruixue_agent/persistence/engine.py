...
为什么用同步而不是 async(deer-flow 用的是 async):
    deer-flow 是 web 服务:大量并发请求、每个请求大部分时间在等 IO → async 能让一个线程服务上千请求。
...
# docker/.env 在仓库根(docker 编排属于仓库级,不属于 backend)——
...
def database_url() -> str:
    ...
    # 主机名必须可配:本机开发连回环,但容器里的 localhost 是容器自己,
    # 得连服务名 postgres。写死的话 app 一进容器就连不上数据库。
    #
    # 注意:默认值用 127.0.0.1 而不是 localhost —— 实测差 200 倍(0.05s vs 10.13s):
...
def get_engine() -> Engine:
    ...
    池大小怎么定 —— 要和实际并发对齐,不是拍个数:

        注意:2026-08-10 实测修正原来这里按"每 worker 满池 30 条"推算,
          得出"最多 3~4 个 worker"。压测实测发现这个算式过于保守约一倍:

              4 个 worker 压测中,PG 实际连接数只有 54/100(不是预测的 120)

          原因:SQLAlchemy 的连接池是懒建的 —— pool_size=10 是"最多留 10 条
          空闲",不是"启动就占 10 条"。实际用多少建多少。
          所以那个算式是最坏情况上界,不是实际用量;拿它当扩容红线会
          白白限制住本可以加的 worker。

        真正的红线不是连接数,是内存—— 见 runs.py 里 MAX_CONCURRENT_RUNS
