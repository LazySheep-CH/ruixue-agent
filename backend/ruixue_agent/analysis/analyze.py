...
预置工具的能力有边界,但边界内覆盖了绝大多数真实需求,而且**工具仍然只读**:
...
**① 缺失不当零。** 缺失值参与均值会把结论拉偏,而且不报错。
所有统计都只在非缺失样本上算,并**同时报出样本量** ——
"3 行数据算出来的均值"和"300 行算出来的"分量完全不同,不报 n 等于隐瞒。

**② 对比要报方向,不只报大小。** 「偏差 8%」没有信息量;
「实测比预测**低** 8%」才能指向原因(降解慢了 → 可能是配方/温度/微生物)。
"""
...
def compare_with_model(ds: Dataset, model: str) -> str:
    ...
    这是整个上传功能的**核心价值**:别人做不了这件事,因为别人没有这三个
...
def detect_outliers(ds: Dataset, z: float = OUTLIER_Z) -> str:
    ...
    "更严谨一点",是**在我们的场景下前者根本不能用**。

    ⚠ 样本量小于 5 时不做判断:5 个点算不出有意义的离散度,
      硬报"异常"会把正常波动说成问题。
    """
    lines: list[str] = []
    for key, label in [
