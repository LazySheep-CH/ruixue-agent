"""对上传数据集的分析:概览、与预测模型对比、离群检测、国标符合性。

做成预置只读工具而不是给 agent 代码执行:没有沙箱的代码执行等于把提示注入
升级成远程代码执行,而沙箱是个以月计的工程。这几个工具只读 PG 里的数据,
不写文件、不执行用户表达式,agent 仍然自己决定调用顺序和解读。

统计口径:
- 缺失值不参与统计,且样本量 n 必须随结果一起报 —— 3 行算出的均值和
  300 行的不是一回事。
- 对比报方向不只报大小:"实测比预测低 8%"才能指向原因,"偏差 8%"不能。
- 离群检测用中位数 + MAD。均值 + 标准差在小样本下会被离群点自己撑大阈值:
  实测 8 行降解率混入一个 88.0,z 分数法漏判(49.3 < 2.5x20.652),
  MAD 法修正 z=11.4 当场命中。田间数据全是小样本,这不是风格差异。
"""
