...
# 不设的话默认是无限等—— 对方卡住,你这个请求就永远挂着,占着的资源永远不放。
...
def create_model(name: str, **overrides) -> BaseChatModel:
    ...
    目前唯一的真实用途是评测:评测要把 temperature 压到 0。
    原因很实在 —— 温度不设时用的是服务端默认(DeepSeek 是 1.0),
    实测同一版本连跑三轮通过率 84.8% / 93.9% / 97.0%,极差 12.1% ≈ 4 道题。
    也就是说这把尺子测不出任何小于 4 道题的改进,拿它做版本对比毫无意义。

    注意:温度 0 不等于完全确定:服务端批处理、MoE 路由都会带来残余抖动。
      它只是把最大的那个来源(采样)去掉。
    注意:评测跑 temperature=0,而线上跑配置里的值 —— 两者不一致时,评测结论是
    ...
    for model in config["models"]:
        if model["name"] == name:
            ...
            params = {
                ...
                # 关掉 SDK 自带的重试 —— 重试统一交给中间件做,避免重试叠加:
