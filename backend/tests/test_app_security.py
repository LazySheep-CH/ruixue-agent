...
建 agent 挪进了 lifespan,所以导入 main、不进入 lifespan就不会连库。
我们用不带 with 的 TestClient发请求 —— lifespan 不触发,_agent 保持 None。
而这些请求都在碰到 agent 之前就被拦下(认证失败/输入超长),
...
# 下面几个用正确的钥匙(见上方 fixture),让认证先通过,单独验证"输入校验"这一关。
...
def test_valid_input_reaches_agent_and_returns_200(monkeypatch):
    ...
    我们不想在单测里真连库/真调 LLM,于是用替身:monkeypatch 把 main 里的
    _agent 临时换成一个假 agent(只会返回一句固定答案)。monkeypatch 是 pytest
    的夹具,它做的替换只在这条测试内生效,测完自动还原,不污染别的测试。

    这条测试证明的是整条链路通了:认证放行 -> 输入校验放行 -> 命名空间拼接 ->
