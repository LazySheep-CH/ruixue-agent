...
1. 绝不存明文,也绝不用 md5/sha1 直接存。密码哈希要慢才安全 ——
   bcrypt 每次校验故意耗时几十毫秒,让暴力破解不可行;而 sha256 快到一秒能算几亿次。

2. bcrypt 有 72 字节上限:超过部分被静默截断,即"密码前72字节相同就算同一个密码"。
   对策(bcrypt 场景的通行做法):先用 sha256 把任意长度压成固定 32 字节、再 base64 → 永远不超限,
   且整个密码都参与运算。
...
def verify_password(password: str, hashed: str) -> bool:
    """校验密码。用 bcrypt.checkpw —— 它是常数时间比较,防时序攻击。"""
    try:
